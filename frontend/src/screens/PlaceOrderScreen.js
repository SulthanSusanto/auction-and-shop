...
const PlaceOrderScreen = ({ history }) => {
  ...
  cart.cartItems.forEach((item) => {
    if (item.userId === userInfo._id) cart.myCartItems.push(item)
  })
  ...
  cart.itemsPrice = addDecimals(
    cart.myCartItems.reduce((acc, item) => acc + item.qty * item.price, 0)
  )
          ...
          <ListGroup variant='flush'>
            ...
            <ListGroup.Item>
              ...
              {cart.cartItems.length === 0 ? (
                ...
                <ListGroup variant='flush'>
                  {cart.myCartItems.map((item, index) => (
